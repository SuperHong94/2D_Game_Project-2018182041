    ...
    def draw(self):
        ...
        if (self.fire):
            if self.enemyType == 0:
                ...
                if main_state.collideBullet(self, main_state.player):
                ...
                t = self.timer / 500
                self.bX = ((1 - t) ** 3) * self.sTargetX + 3 * (cX + (i * dx)) * t * ((1 - t) ** 2) + 3 * (
                            cX + (i * (-1) * dx1)) * (t ** 2) * (1 - t) + (t ** 3) * self.targetX
                self.bY = ((1 - t) ** 3) * self.sTargetY + 3 * (cY * 0.25) * t * ((1 - t) ** 2) + 3 * (cY * 0.5) * (
                            t ** 2) * (1 - t) + (t ** 3) * self.targetY
                self.bulletImage.clip_draw(0, 0, 10, 10, self.bX, self.bY)
                if main_state.collideBullet(self, main_state.player):
                    explosions = explosion.Explosion(self.bX, self.bY )
                    game_world.add_object(explosions, 1)
